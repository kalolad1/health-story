...
class Patient(models.Model):
    ...
    # Admin mode is set to true when a physician logs in. This allows the
    # physician to modify information or add new health encounters.
    ...
    def is_physician_code_valid(self, physician_code):
        ...
        A physician code is generated by the user and expires after 1 hour (3600
        seconds). After that time, the physician will no longer be able to
        access the patients health information.
        """
        ...
        if not self.physician_code_created or \
                time_elapsed.total_seconds() > expiration_time:
            return False
    ...
    def get_all_patient_info(self):
        ...
        Iterates through all of the patients health encounters and pulls out the
        medications and conditions. Call this function when the user logs in for
        the first time and store with cookie.
    ...
    def get_info_for_ui(self, page):
        """This function organizes information into an array to be used in UI.
        ...
        is its value. Tuples instead of dicts were chosen to retain ordering
        ...
        if page == MPConstants.DEMOGRAPHICS:
            data = [('Date of Birth',
                     self.get_date_of_birth_formatted()),
                    ('Sex', self.sex),
                    ('Race', self.race),
                    ('Email', self.email)]

        elif page == MPConstants.VITALS:
            data = [('Weight (lb)', self.weight.lb),
                    ('Height (m)', self.get_height_formatted())]
...
class HealthEncounter(models.Model):
    ...
    type_of_encounter = models.CharField(
        max_length=100, choices=choices.TYPE_OF_HEALTH_ENCOUNTER
    )
    ...
    def __str__(self):
        ...
        """
        return '{} with {} on {}'.format(self.patient.get_full_name(),
                                         self.physician, self.date_formatted())
    ...
    def formatted_description(self):
        ...
        description += "Conditions identified: {}.\n".format(
            self.get_cs_string_from_query_set(self.conditions.all())
        )
        description += "Medications prescribed: {}.\n".format(
            self.get_cs_string_from_query_set(self.medications.all())
        )
        description += self.description
        return description

    @staticmethod
    def get_cs_string_from_query_set(query_set):
        query_string = ""
...
class Relative(models.Model):
    ...
    type_of_relationship = models.CharField(
        max_length=100, choices=choices.TYPE_OF_RELATIONSHIP, blank=True
    )
    conditions = models.ManyToManyField('Condition')
